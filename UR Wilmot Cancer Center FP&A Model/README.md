...
Cover — model context, data sources, color/formatting conventions
Assumptions — every driver in one place: rates, ramps, headcount, F&A indirect cost rates, scenario selector
BudgetVsActual — FY2025 monthly P&L with $ and % variance and commentary
ProgramP&L — departmental income statement: revenue → comp → supplies → F&A recovery → net margin
Forecast5Yr — FY2026–FY2030 driver-based forecast (accruals × per-patient revenue × growth rates)
TrialDecision — per-trial NPV/IRR/break-even analysis with 5×5 accrual-vs-revenue sensitivity table
GrantF&A — grant portfolio + Indirect Cost (F&A) recovery analysis (federal vs industry vs foundation)
KPIDashboard — executive KPI dashboard: margin, cost per patient, RVU, productivity, days cash
Scenarios — Base / Upside / Downside stress test; flip C6 on Assumptions to switch scenarios live
ICSummary — one-pager investment-committee deliverable: thesis, hurdles, recommendation
...
ColorMeaningBlue textHardcoded inputs / drivers — user-editableBlack textFormulas and calculations within the same tabGreen textCross-sheet links pulling from other tabsYellow fillKey assumption — attention requiredLight-blue fillSub-totals and grand totalsGreen text (variance)Favorable variance vs. budgetRed text (variance)Unfavorable variance vs. budget
Disclaimer
